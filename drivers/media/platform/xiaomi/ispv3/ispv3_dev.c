...
static int ispv3_init_rpmsg_callback(struct ispv3_data *pdata)
{
	...
	ret = mfd_add_devices(pdev, PLATFORM_DEVID_NONE, &ispv3_rpmsg_spi_cell,
			      1, NULL, 0, NULL);
	if (ret) {
...
static int ispv3_init_v4l2(struct pci_dev *pdev)
{
	...
	ret = mfd_add_devices(&pdev->dev, PLATFORM_DEVID_NONE, &ispv3_v4l2_cell,
			      1, NULL, 0, NULL);
	if (ret) {
...
static int ispv3_init_rpmsg(struct ispv3_data *pdata)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(ispv3_rpmsg_res); idx++) {
		...
		case RPMSG_RAM:
			...
			ispv3_rpmsg_res[idx].start =
				pci_resource_start(pdev, BAR_2) + OCRAM_OFFSET;
			ispv3_rpmsg_res[idx].end =
				pci_resource_start(pdev, BAR_2) + OCRAM_OFFSET +
				RPMSG_SIZE - 1;
			break;
		case RPMSG_IRQ:
...
static int ispv3_init_camera(struct pci_dev *pdev, struct ispv3_data *priv)
{
	...
	ret = mfd_add_devices(&pdev->dev, PLATFORM_DEVID_NONE, &ispv3_cam_cell,
			      1, NULL, 0, NULL);
	if (ret) {
...
static void ispv3_init_bar_resource(struct pci_dev *pdev,
				    struct ispv3_data *priv)
{
	int idx = 0;
	int bar_idx = 0;
...
static bool ispv3_alloc_irq_vectors(struct pci_dev *pdev,
				    struct ispv3_data *priv)
{
...
static int ispv3_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	int idx, ret;
	...
	if (pci_resource_flags(pdev, BAR_0) & IORESOURCE_MEM) {
		...
		if (!priv->base) {
			dev_err(&pdev->dev,
				"Failed to map the register of BAR0!\n");
...
int ispv3_pci_init(struct ispv3_data *data)
{
	...
	if (ret < 0) {
		dev_err(data->dev,
			"ispv3 PCIE enumeration failed! Not PCIE mode\n");
		return ret;
	}
...
static int ispv3_get_dt_regulator_info(struct ispv3_data *data)
{
	...
	for (i = 0; i < data->num_rgltr; i++) {
		ret = of_property_read_string_index(of_node, "regulator-names",
						    i, &data->rgltr_name[i]);
...
static int ispv3_get_dt_clk_info(struct ispv3_data *data)
{
	...
	ret = of_property_read_u32_index(of_node, "clock-rates", 0,
					 &data->clk_rate);
	if (ret) {
	...
	data->clk_rate = (data->clk_rate == 0) ? (int32_t)ISPV3_NO_SET_RATE :
						 data->clk_rate;
			      ...
			      struct device *dev)
{
	...
	sensor_pctrl->gpio_state_active = pinctrl_lookup_state(
		sensor_pctrl->pinctrl, ISPV3_PINCTRL_STATE_DEFAULT);
	...
	sensor_pctrl->gpio_state_suspend = pinctrl_lookup_state(
		sensor_pctrl->pinctrl, ISPV3_PINCTRL_STATE_SLEEP);
...
static int ispv3_conf_gpio(struct ispv3_data *data)
{
	...
	ret = devm_gpio_request(data->dev, data->gpio_isolation,
				"isolation_gpio");
	if (ret) {
	...
	if (ret) {
		dev_err(data->dev,
			"cannot set direction for isolation gpio[%d]\n",
			data->gpio_isolation);
		return ret;
	}
	...
	ret = devm_gpio_request(data->dev, data->gpio_swcr_reset,
				"reset_gpio1");
	if (ret) {
	...
	ret = devm_gpio_request(data->dev, data->gpio_fan_en, "fan_en_gpio");
	if (ret) {
	...
	if (ret) {
		dev_err(data->dev,
			"cannot set direction for fan53526 enable gpio[%d]\n",
			data->gpio_fan_en);
		return ret;
	}
	...
	ret = devm_gpio_request(data->dev, data->gpio_int0,
				"interrupt_gpio_cam");
	if (ret) {
	...
	if (ret) {
		dev_err(data->dev,
			"cannot set direction for interrupt0 gpio[%d]\n",
			data->gpio_int0);
		return ret;
	}
	...
	ret = devm_gpio_request(data->dev, data->gpio_int1,
				"interrupt_gpio_power");
	if (ret) {
	...
	if (ret) {
		dev_err(data->dev,
			"cannot set direction for interrupt1 gpio[%d]\n",
			data->gpio_int1);
		return ret;
	}
...
static int ispv3_spi_probe(struct spi_device *spi)
{
	...
	if (data->interface_type == ISPV3_SPI) {
		...
		ret = mfd_add_devices(dev, PLATFORM_DEVID_NONE, &ispv3_cam_cell,
				      1, NULL, 0, NULL);
		if (ret) {
...
static const struct spi_device_id ispv3_spi_device_id[] = { { "ispv3_spi", 0 },
							    {} };
