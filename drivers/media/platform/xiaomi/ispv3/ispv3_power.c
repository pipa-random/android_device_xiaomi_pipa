...
static int ispv3_clk_enable(struct ispv3_data *data)
{
	...
	if (ret) {
		dev_err(data->dev, "enable failed for %s: ret(%d)",
			data->clk_name, ret);
		return ret;
	}
...
static int ispv3_set_pci_config_space(struct ispv3_data *data, bool save)
{
	...
	if (save) {
		...
		if (data->saved_state)
			pci_load_and_free_saved_state(data->pci,
						      &data->saved_state);
		else
...
retry:
	...
	if (ret) {
		dev_err(data->dev,
			"Failed to %s PCI link with default option, err = %d\n",
...
int ispv3_suspend_pci_link(struct ispv3_data *data)
{
	...
	if (ret) {
		dev_err(data->dev,
			"Failed to set link down status, err =  %d\n", ret);
		return ret;
	}
...
int ispv3_power_on(struct ispv3_data *data)
{
	...
	if (data->soc_id == ISPV3_SOC_ID_SM8475) {
		...
		if (ret) {
			dev_err(data->dev,
				"ISPV3_RGLTR_S11B regulator enable failed\n");
			return ret;
		}
		ret = ispv3_regulator_enable(data, ISPV3_RGLTR_S12B);
		if (ret) {
			dev_err(data->dev,
				"ISPV3_RGLTR_S12B regulator enable failed\n");
			return ret;
		}
	}
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_VDD1 regulator enable failed\n");
		return ret;
	}
	udelay(100);
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_L12C regulator enable failed\n");
		return ret;
	}
	udelay(1);
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_VDD2 regulator enable failed\n");
		return ret;
	}
	udelay(100);
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_VDDR regulator enable failed\n");
		return ret;
	}
	udelay(100);
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_L10C regulator enable failed\n");
		return ret;
	}
	...
	if (ret) {
		dev_err(data->dev,
			"ISPV3_RGLTR_MCLK regulator enable failed\n");
		return ret;
	}
