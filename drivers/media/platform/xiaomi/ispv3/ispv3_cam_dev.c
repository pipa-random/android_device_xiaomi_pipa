...
static int ispv3_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	ret = io_remap_pfn_range(vma, vma->vm_start, paddr >> PAGE_SHIFT, vsize,
				 vma->vm_page_prot);

	dev_dbg(priv->dev,
		"vma=%ps, vma->vm_start=0x%lx, phys=0x%lx, vsize=%lu, psize=%lu, rv=%d\n",
...
static int isp_v4l2_device_setup(struct ispv3_v4l2_dev *priv)
{
	...
	priv->v4l2_dev = kzalloc(sizeof(*priv->v4l2_dev), GFP_KERNEL);
	if (!priv->v4l2_dev)
		return -ENOMEM;
	...
	priv->v4l2_dev->mdev =
		kzalloc(sizeof(*priv->v4l2_dev->mdev), GFP_KERNEL);
