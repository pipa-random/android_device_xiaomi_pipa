...
void print_buffer(int *buffer, int length, int line_num)
{
	...
	for (i = 0; i < length; i = i + tmpline) {
		cnt = 0;
		for (j = 0; j < tmpline; j++) {
			cnt += snprintf(tmpbuf + cnt, tmplen - cnt, "%d,",
					buffer[i + j]);
...
void show_buffer(int *buffer, int length, int line_num)
{
	...
	if (buffer && length) {
		...
		do {
			cnt += snprintf(line_buf + cnt, MAX_SHOW - cnt, "%d,",
					buffer[i++]);
			if (((i % line_size) == 0) || (i == length) ||
			    (cnt >= MAX_SHOW)) {
...
int fts_test_read(u8 addr, u8 *readbuf, int readlen)
{
	...
	for (i = 1; i < packet_num; i++) {
		...
		if (fts_ftest->ts_data->bus_type == BUS_TYPE_SPI)
			...
			FTS_TEST_ERROR("unknown bus type:%d",
				       fts_ftest->ts_data->bus_type);
		if (ret < 0) {
			FTS_TEST_ERROR("read buffer fail");
			return ret;
		}
...
int enter_work_mode(void)
{
	...
	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		...
		if (ret >= 0) {
			...
			for (j = 0; j < 20; j++) {
				ret = fts_test_read_reg(DEVIDE_MODE_ADDR,
							&mode);
				if ((ret >= 0) && (0x00 == mode)) {
					FTS_TEST_INFO(
...
static int read_rawdata(struct fts_test *tdata, u8 off_addr, u8 off_val,
			u8 rawdata_addr, int byte_num, int *data)
{
	int ret = 0;
...
int mapping_switch(u8 mapping)
{
	...
	if (tdata->v3_pattern) {
		...
		if (val != mapping) {
			ret = fts_test_write_reg(FACTORY_REG_NOMAPPING,
						 mapping);
			if (ret < 0) {
...
int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		if ((ret >= 0) && (retval == short_state))
			...
			FTS_TEST_DBG("reg%x=%x,retry:%d", short_state_reg,
				     short_state, i);
	}
...
void show_data_mc_sc(int *data)
{
	...
	FTS_TEST_SAVE_INFO("\n");
#else
	show_buffer(data, fts_ftest->sc_node.node_num,
		    fts_ftest->sc_node.node_num);
#endif
}
/* mc_sc end*/
...
void fts_test_save_fail_result(struct fts_test *tdata, char *prefix,
			       char *suffix, char *buf, int len)
{
	char file_name[128];

	if (false == tdata->result) {
		snprintf(file_name, 128, "%s_%ld_%ld%s", prefix,
			 (long)tdata->tv.tv_sec, (long)tdata->tv.tv_usec,
			 suffix);
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	/* line 1 */
	csv_length +=
		snprintf(csv_buffer + csv_length, CSV_BUFFER_LEN - csv_length,
	...
	for (i = 0; i < td->item_count; i++) {
		...
		for (j = 1; j <= data_count; j++) {
			...
			if (tdata->func->hwtype == IC_HW_MC_SC) {
				...
				if ((info->code == CODE_M_RAWDATA_TEST) &&
				    (data_count == 1)) {
					index = 2;
				}
				...
				if ((info->code == CODE_M_SCAP_CB_TEST) ||
					...
					index = fts_test_get_item_count_scap_csv(
						j);
				}
			}

			line2_length +=
				snprintf(line2_buffer + line2_length,
					 CSV_LINE2_BUFFER_LEN - line2_length,
					 "%s, %d, %d, %d, %d, %d, ", info->name,
					 info->code, tx, rx, start_line, index);
			start_line += tx;
			csv_item_count++;
		}
	}
...
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
	fts_test_save_fail_result(tdata, "testdata_fail", ".csv", csv_buffer,
				  csv_length);
#endif
...
static int get_channel_num(struct fts_test *tdata)
{
	...
	if (IC_HW_SC == tdata->func->hwtype) {
		...
		ret = get_tx_rx_num(FACTORY_REG_KEY_NUM_SC, &rx_num,
				    KEY_NUM_MAX);
		if (ret < 0) {
	...
	if (IC_HW_MC_SC == tdata->func->hwtype) {
		if (tdata->v3_pattern) {
			ret = get_tx_rx_num(FACTORY_REG_CHX_NUM_NOMAP, &tx_num,
					    TX_NUM_MAX);
			if (ret < 0) {
			...
			ret = get_tx_rx_num(FACTORY_REG_CHY_NUM_NOMAP, &rx_num,
					    TX_NUM_MAX);
			if (ret < 0) {
	...
	FTS_TEST_INFO("node_num:%d, tx:%d, rx:%d, key:%d", tdata->node.node_num,
		      tdata->node.tx_num, tdata->node.rx_num,
		      tdata->node.key_num);
	return 0;
}
...
static struct attribute *fts_test_attributes[] = { &dev_attr_fts_test.attr,
						   NULL };
...
int fts_test_init(struct fts_ts_data *ts_data)
{
	...
	ret = sysfs_create_group(&ts_data->dev->kobj,
	...
	fts_ftest->proc_csv = proc_create_data("fts_test_csv", 0777, NULL,
					       &fts_proccsv_fops, fts_ftest);
...
/*****************************************************************************/
/* N17 code for HQ-291087 by liunianliang at 2023/5/29 start */
//-------------------/proc/tp_data_dump begin-----------------
struct lct_tp_data_dump *lct_tp_data_dump_p;
...
static int tp_data_dump_proc_open(struct inode *inode, struct file *file)
{
	...
	//malloc buffer
	lct_tp_data_dump_p->rawdata =
		(int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
	if (IS_ERR_OR_NULL(lct_tp_data_dump_p->rawdata)) {
	...
	//get rawdata
	ret = lct_tp_get_rawdata(lct_tp_data_dump_p->rawdata,
				 tx_num * rx_num * 2);
	if (ret < 0) {
	...
	//get diffdata
	ret = lct_tp_get_diffdata(lct_tp_data_dump_p->diffdata,
				  tx_num * rx_num * 2);
	if (ret < 0) {
		FTS_TEST_ERROR("lct_tp_get_rawdata() failed, ret = %d", ret);
		goto err_get_data_fail;
...
const struct proc_ops fts_tp_data_dump_ops = {
	.proc_open = tp_data_dump_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
//-------------------/proc/tp_data_dump end-------------------

//-------------------/proc/android_touch/self_test begin-----------------
u8 ito_test_status;
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	seq_printf(file, "0%c-1%c-2%c-3%c-4%c\n",
		   test_result_bmp[I2C_COMMUNICATION], test_result_bmp[RAWDATA],
		   test_result_bmp[OPEN_DATA], test_result_bmp[SHORT_DATA],
...
static int fts_ito_test_open(struct inode *inode, struct file *file)
{
	...
	if (ts_data->lockdown_info[7] == 0x01) {
		sprintf(fwname, "Conf_MultipleTest_%s_mask0x%02x.ini",
			ts_data->vendor, ts_data->lockdown_info[7]);
	} else {
		sprintf(fwname, "Conf_MultipleTest_%s.ini", ts_data->vendor);
	}
	...
	case 2:
		if (fts_ftest->result == TEST_RESULT_PASS) {
			FTS_TEST_INFO("ITOtest status = 2\n\
            ITOtest result: Pass\n\
            ITOtest save: Failed\n");
		} else {
			FTS_TEST_INFO("ITOtest status = 2\n\
	...
	case 3:
		if (fts_ftest->result == TEST_RESULT_PASS) {
			FTS_TEST_INFO("ITOtest status = 3\n\
                ITOtest result: Pass\n\
	...
	case 4:
		if (fts_ftest->result == TEST_RESULT_PASS) {
			FTS_TEST_INFO("ITOtest status = 4\n\
                ITOtest result: Pass\n\
                ITOtest save: Succeeded\n\
                ITOtet end\n");
		} else {
