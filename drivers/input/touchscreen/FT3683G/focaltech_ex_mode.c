...
/* N17 code for HQ-291087 by liunianliang at 2023/5/29 start */
static ssize_t fts_earphone_mode_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
...
static ssize_t fts_earphone_mode_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	int ret = 0;
				   ...
				   const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (ts_data->edge_mode == 0 || ts_data->edge_mode == 1 ||
		    ts_data->edge_mode == 2) {
...
/* N17 code for HQ-291087 by liunianliang at 2023/5/29 start */
static DEVICE_ATTR(fts_earphone_mode, S_IRUGO | S_IWUSR, fts_earphone_mode_show,
		   fts_earphone_mode_store);
...
/* N17 code for HQ-291711 by liunianliang at 2023/06/03 start */
static int charger_mode_noti(struct notifier_block *nb, unsigned long event,
			     void *v)
{
	struct fts_ts_data *ts_data =
		container_of(nb, struct fts_ts_data, tcpc_nb);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			if (ts_data->earphone_mode) {
				FTS_DEBUG("exit earphone mode");
				ret = fts_ex_mode_switch(MODE_EARPHONE,
							 DISABLE);
				if (ret >= 0) {
					ts_data->earphone_mode = DISABLE;
				}
...
int earphone_mode_noti(struct notifier_block *nb, unsigned long event, void *v)
{
	struct fts_ts_data *ts_data =
		container_of(nb, struct fts_ts_data, earphone_nb);
