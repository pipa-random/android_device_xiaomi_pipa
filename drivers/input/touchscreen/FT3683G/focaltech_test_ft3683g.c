...
static int short_test_ch_to_all(struct fts_test *tdata, int *res, u8 *ab_ch,
				bool *result)
{
	int ret = 0;
	int i = 0;
...
static int short_test_ch_to_gnd(struct fts_test *tdata, int *res, u8 *ab_ch,
				bool *result)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < ab_ch_num; i++) {
		if (res[i] < min_cg) {
			*result = false;
			...
			if (ab_ch[i + 1] <= tx_num) {
				FTS_TEST_SAVE_INFO("Tx%d with GND:",
						   ab_ch[i + 1]);
			} else {
				FTS_TEST_SAVE_INFO("Rx%d with GND:",
						   (ab_ch[i + 1] - tx_num));
			}
...
static int short_test_ch_to_ch(struct fts_test *tdata, int *res, u8 *ab_ch,
			       bool *result)
{
	int ret = 0;
	int i = 0;
...
static int get_cb_ft5672(int *cb_buf, int byte_num, bool is_cf)
{
	...
	for (i = 0; i < byte_num; i++) {
		if (is_cf)
			cb_buf[i] = (cb[i] & 0x80) ? -((cb[i] & 0x3F)) :
						     (cb[i] & 0x3F);
		else
			cb_buf[i] = (cb[i] & 0x80) ? -((cb[i] & 0x7F)) :
						     (cb[i] & 0x7F);
	}
...
static int scap_cb_cc(struct fts_test *tdata, int *scap_cb, bool *result)
{
	...
	if (thr->basic.scap_cb_wp_on_check && fw_wp_check) {
		...
		ret = get_cb_ft5672(scb_cf, byte_num, true);
		if (ret < 0) {
		...
		for (i = 0; i < tdata->sc_node.node_num; i++) {
			...
			if ((rx_check && (i < tdata->sc_node.rx_num)) ||
				...
				if ((scb_cf[i] < thr->scap_cb_on_cf_min[i]) ||
					...
					FTS_TEST_SAVE_ERR(
						"test2 fail,CH%d=%5d,range=(%5d,%5d)\n",
						i + 1, scb_cf[i],
...
static int scap_cb_ccbypass(struct fts_test *tdata, int *scap_cb, bool *result)
{
	...
	if (thr->basic.scap_cb_wp_on_check && fw_wp_check) {
		...
		scb_tmp[tdata->sc_node.node_num + tdata->sc_node.rx_num] =
			scap_gcb_tx;
		...
		if (rx_check &&
		    ((scap_gcb_rx < thr->basic.scap_cb_on_gcb_min) ||
		     (scap_gcb_rx > thr->basic.scap_cb_on_gcb_max))) {
			FTS_TEST_SAVE_ERR(
				"test fail,gcb_rx:%5d,range=(%5d,%5d)\n",
				scap_gcb_rx, thr->basic.scap_cb_on_gcb_min,
		...
		if (tx_check &&
		    ((scap_gcb_tx < thr->basic.scap_cb_on_gcb_min) ||
		     (scap_gcb_tx > thr->basic.scap_cb_on_gcb_max))) {
			FTS_TEST_SAVE_ERR(
				"test fail,gcb_tx:%5d,range=(%5d,%5d)\n",
				scap_gcb_tx, thr->basic.scap_cb_on_gcb_min,
				thr->basic.scap_cb_on_gcb_max);
			tmp_result = false;
		}
	...
	if (thr->basic.scap_cb_wp_off_check && fw_wp_check) {
		...
		if (rx_check &&
		    ((scap_gcb_rx < thr->basic.scap_cb_off_gcb_min) ||
		...
		if (tx_check &&
		    ((scap_gcb_tx < thr->basic.scap_cb_off_gcb_min) ||
	...
	if (thr->basic.scap_cb_hi_check && hov_high) {
		...
		if (rx_check &&
		    ((scap_gcb_rx < thr->basic.scap_cb_hi_gcb_min) ||
		...
		if (tx_check &&
		    ((scap_gcb_tx < thr->basic.scap_cb_hi_gcb_min) ||
...
static int ft5672_spi_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
...
static int ft5672_uniformity_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (thr->basic.uniformity_check_tx) {
		...
		for (row = 0; row < tx_num; row++) {
			for (col = 1; col < rx_num; col++) {
				...
				deviation =
					abs(rawdata[offset] - rawdata[offset2]);
	...
	if (thr->basic.uniformity_check_min_max) {
		...
		if (uniform < thr->basic.uniformity_min_max_hole) {
			result3 = false;
			FTS_TEST_SAVE_ERR(
				"min_max out of range, set value: %d\n",
...
static int ft5672_short_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (ch_num >= SC_NUM_MAX) {
		FTS_TEST_SAVE_ERR("sc_node ch_num(%d)>max(%d)", ch_num,
				  SC_NUM_MAX);
		goto test_err;
	}
	...
	if (!ca_result) {
		...
		if (ret < 0) {
			FTS_TEST_SAVE_ERR(
				"short test of channel to gnd fails\n");
			goto test_err;
		}
		...
		if (ret < 0) {
			FTS_TEST_SAVE_ERR(
				"short test of channel to channel fails\n");
			goto test_err;
		}
	}
...
static int ft5672_noise_test(struct fts_test *tdata, bool *test_result)
{
	...
	FTS_TEST_INFO("fre:%d,data_sel:%d,reg1a:%d,reg1b:%d", fre, data_sel,
		      reg1a_val, reg1b_val);
	...
	if (thr->basic.noise_polling) {
		...
		for (i = tdata->fre_num - 1; i > 0; i--) {
			ret = get_noise_ft5672(
				tdata, &noise[tdata->node.node_num * i], i);
			if (ret < 0) {
	...
	/* compare */
	tmp_result =
		compare_array(noise, thr->noise_min, thr->noise_max, false);
...
static int malloc_item_data(struct fts_test *tdata)
{
	...
	tdata->item3_data =
		fts_malloc(tdata->sc_node.node_num * sizeof(int) * 6);
	if (!tdata->item3_data) {
		FTS_TEST_SAVE_ERR("memory malloc fails");
		return -ENOMEM;
	}
	...
	tdata->item4_data =
		fts_malloc(tdata->sc_node.node_num * sizeof(int) * 3);
	if (!tdata->item4_data) {
		FTS_TEST_SAVE_ERR("memory malloc fails");
		return -ENOMEM;
	}
	...
	tdata->item6_data =
		fts_malloc(tdata->node.node_num * sizeof(int) * tdata->fre_num);
	if (!tdata->item6_data) {
		FTS_TEST_SAVE_ERR("memory malloc fails");
		return -ENOMEM;
	}
...
static int start_test_ft5672(void)
{
	...
	/* spi test */
	if (1) {
		ret = ft5672_spi_test(tdata, &temp_result);
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult =
				RESULT_NG;
		} else {
			fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult =
				RESULT_PASS;
	...
	if (true == test_item->rawdata_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_RAWDATA_TEST].testresult =
				RESULT_NG;
		} else {
			fts_ftest->test_item[FTS_RAWDATA_TEST].testresult =
				RESULT_PASS;
	...
	if (true == test_item->short_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_SHORT_CIRCUIT_TEST].testresult =
				RESULT_NG;
		} else {
			fts_ftest->test_item[FTS_SHORT_CIRCUIT_TEST].testresult =
				RESULT_PASS;
	...
	if (true == test_item->panel_differ_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_OPEN_TEST].testresult =
				RESULT_NG;
		} else {
			fts_ftest->test_item[FTS_OPEN_TEST].testresult =
				RESULT_PASS;
	...
	if (true == test_item->noise_test) {
		...
		if ((ret < 0) || (false == temp_result)) {
			test_result = false;
			fts_ftest->test_item[FTS_NOISE_TEST].testresult =
				RESULT_NG;
		} else {
			fts_ftest->test_item[FTS_NOISE_TEST].testresult =
				RESULT_PASS;
...
static int param_init_ft5672(void)
{
	...
	get_value_basic("SCapCbTest_ON_Global_Cb_Min",
			&thr->basic.scap_cb_on_gcb_min);
	get_value_basic("SCapCbTest_ON_Global_Cb_Max",
			&thr->basic.scap_cb_on_gcb_max);
	get_value_basic("SCapCbTest_OFF_Global_Cb_Min",
			&thr->basic.scap_cb_off_gcb_min);
	get_value_basic("SCapCbTest_OFF_Global_Cb_Max",
			&thr->basic.scap_cb_off_gcb_max);
	get_value_basic("SCapCbTest_High_Global_Cb_Min",
			&thr->basic.scap_cb_hi_gcb_min);
	get_value_basic("SCapCbTest_High_Global_Cb_Max",
			&thr->basic.scap_cb_hi_gcb_max);
	FTS_TEST_INFO(
		"GCB,On(%d,%d),Off(%d,%d),High(%d,%d)",
	...
	get_value_basic("SCapCbTest_ON_Cf_Cb_Min",
			&thr->basic.scap_cb_on_cf_min);
	get_value_basic("SCapCbTest_ON_Cf_Cb_Max",
			&thr->basic.scap_cb_on_cf_max);
	get_value_basic("SCapCbTest_OFF_Cf_Cb_Min",
			&thr->basic.scap_cb_off_cf_min);
	get_value_basic("SCapCbTest_OFF_Cf_Cb_Max",
			&thr->basic.scap_cb_off_cf_max);
	get_value_basic("SCapCbTest_High_Cf_Cb_Min",
			&thr->basic.scap_cb_hi_cf_min);
	get_value_basic("SCapCbTest_High_Cf_Cb_Max",
			&thr->basic.scap_cb_hi_cf_max);
...
static void save_data_ft5672(char *buf, int *data_length)
{
	...
	/*line 2*/
	cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "TestItem Num, %d, ",
			tdata->csv_item_cnt);
	if (true == test_item->rawdata_test) {
		cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
	...
	if (true == test_item->rawdata_uniformity_test) {
		if (thr->basic.uniformity_check_tx) {
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
					"%s, %d, %d, %d, %d, %d, ",
					"Rawdata Uniformity Test",
					CODE_M_RAWDATA_UNIFORMITY_TEST, tx, rx,
					line_num, 1);
			line_num += tx;
		}
		if (thr->basic.uniformity_check_rx) {
	...
	if (true == test_item->scap_cb_test) {
		...
		if (tdata->csv_item_scb & 0x01) {
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
					"%s, %d, %d, %d, %d, %d, ",
					"SCAP CB Test", CODE_M_SCAP_CB_TEST, 2,
					sc_rx, line_num, 1);
			line_num += 2;
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
					"%s, %d, %d, %d, %d, %d, ",
	...
	if (true == test_item->scap_rawdata_test) {
		if (tdata->csv_item_sraw & 0x01) {
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
					"%s, %d, %d, %d, %d, %d, ",
					"SCAP Rawdata Test",
					CODE_M_SCAP_RAWDATA_TEST, 2, sc_rx,
					line_num, 1);
			line_num += 2;
		}
	...
	if (true == test_item->noise_test) {
		...
		for (i = 1; thr->basic.noise_polling && (i < tdata->fre_num);
		     i++) {
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
					"%s, %d, %d, %d, %d, %d, ",
					"Noise Test", CODE_M_NOISE_TEST, tx, rx,
					line_num, 1 + i);
			line_num += tx;
		}

		cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
				"%s, %d, %d, %d, %d, %d, ", "Null Noise", 41, 1,
				1, line_num, 1);
		line_num += 1;
	}
		...
		if (tdata->csv_item_scb & 0x01) {
			...
			for (i = 0; i < tdata->sc_node.rx_num; i++)
				cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
						"%d,", tmp_data[i]);
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "\n");
			for (i = tdata->sc_node.rx_num;
			     i < tdata->sc_node.node_num; i++)
				cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt,
						"%d,", tmp_data[i]);
			cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "\n");
	...
	if (true == test_item->noise_test && tdata->item6_data) {
		...
		for (j = 1; thr->basic.noise_polling && (j < tdata->fre_num);
		     j++) {
