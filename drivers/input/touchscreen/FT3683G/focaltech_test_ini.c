...
struct ini_ic_type ic_types[] = {
	...
	{ "FT5526_003", 0x40020082 },  { "FT5426_003", 0x40030082 },
	{ "FT3427G_003", 0x40040082 }, { "FT3427_003", 0x40050082 },
	{ "FT5446_003", 0x40000082 },  { "FT5446_Q03", 0x40000082 },
	{ "FT5446_P03", 0x55060081 },  { "FT5446DQS-W01", 0x40000082 },

	{ "FT5452", 0x55000081 },      { "FT3518", 0x55010081 },
	{ "FT3558", 0x55020081 },      { "FT3528", 0x55030081 },
	{ "FT5536", 0x55040081 },      { "FT3418", 0x55070081 },
	{ "FT5536L", 0x55080081 },
	...
	{ "FT8201", 0x87010010 },      { "FT7250", 0x8702001A },

	{ "FT8006U", 0x8900000B },     { "FT8006S", 0x8901000B },
	{ "FT8006S-AA", 0x9B000019 },  { "FT8016", 0x9B01001D },

	{ "FT8719", 0x8E00000D },      { "FT8615", 0x9100000F },

	{ "FT8739", 0x8D00000E },

	{ "FT8006P", 0x93000011 },     { "FT7120", 0x9E00001B },
	...
	{ "FT6346U", 0x65050085 },     { "FT6146", 0x65060085 },
	{ "FT6346G", 0x65070085 },

	{ "FT5726_V03", 0x580C0086 },  { "FT5726_003", 0x580C0086 },

	{ "FT3618", 0x59010087 },      { "FT5646", 0x59020087 },
	{ "FT3A58", 0x59030087 },      { "FT3B58", 0x59040087 },
	{ "FT3D58", 0x59050087 },      { "FT5A36", 0x59060087 },
	{ "FT5B36", 0x59070087 },      { "FT5D36", 0x59080087 },
	{ "FT5A46", 0x59090087 },      { "FT5B46", 0x590A0087 },
	{ "FT5D46", 0x590B0087 },      { "FT5936", 0x590C0087 },
	{ "FT5946", 0x590D0087 },
	...
	for (i = 0; i < ini->section_num; i++) {
		...
		for (j = 0; j < section->keyword_num; j++) {
			...
			if (key_len == strlen(keyword->name)) {
				if (0 == fts_strncmp(keyword->name, key_name,
						     key_len)) {
...
static int get_test_threshold_mc_sc(void)
{
	...
	fts_init_buffer(thr->mcap_cmb_max, thr->basic.mcap_cmb_max, node_num,
			false, 0, 0);
	fts_init_buffer(thr->mcap_cmb_min, thr->basic.mcap_cmb_min, node_num,
			false, 0, 0);
	...
	fts_init_buffer(thr->scap_cb_on_max, thr->basic.scap_cb_on_max, sc_num,
			false, 0, 0);
	fts_init_buffer(thr->scap_cb_hi_min, thr->basic.scap_cb_hi_min, sc_num,
			false, 0, 0);
	fts_init_buffer(thr->scap_cb_hi_max, thr->basic.scap_cb_hi_max, sc_num,
			false, 0, 0);
	fts_init_buffer(thr->scap_cb_hov_min, thr->basic.scap_cb_hov_min,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_hov_max, thr->basic.scap_cb_hov_max,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_off_cf_min, thr->basic.scap_cb_off_cf_min,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_off_cf_max, thr->basic.scap_cb_off_cf_max,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_on_cf_min, thr->basic.scap_cb_on_cf_min,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_on_cf_max, thr->basic.scap_cb_on_cf_max,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_hi_cf_min, thr->basic.scap_cb_hi_cf_min,
			sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_cb_hi_cf_max, thr->basic.scap_cb_hi_cf_max,
			sc_num, false, 0, 0);
	...
	fts_init_buffer(thr->scap_rawdata_hi_min,
			thr->basic.scap_rawdata_hi_min, sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_rawdata_hi_max,
			thr->basic.scap_rawdata_hi_max, sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_rawdata_hov_min,
			thr->basic.scap_rawdata_hov_min, sc_num, false, 0, 0);
	fts_init_buffer(thr->scap_rawdata_hov_max,
			thr->basic.scap_rawdata_hov_max, sc_num, false, 0, 0);
	...
	fts_init_buffer(thr->noise_max, thr->basic.noise_max, node_num, false,
			0, 0);
	...
	get_detail_threshold("Rx_Linearity_Max_Tx", true, thr->rx_linearity_max,
			     node_num);
	get_detail_threshold("McapCMBTest_Min_Hole", true, thr->mcap_cmb_min,
			     node_num);
	get_detail_threshold("McapCMBTest_Max_Hole", true, thr->mcap_cmb_max,
			     node_num);
	...
	get_detail_threshold("ScapCB_ON_Max_", true, thr->scap_cb_on_max,
			     sc_num);
	get_detail_threshold("ScapCB_High_Min_", true, thr->scap_cb_hi_min,
			     sc_num);
	get_detail_threshold("ScapCB_High_Max_", true, thr->scap_cb_hi_max,
			     sc_num);
	get_detail_threshold("ScapCB_Hov_Min_", true, thr->scap_cb_hov_min,
			     sc_num);
	get_detail_threshold("ScapCB_Hov_Max_", true, thr->scap_cb_hov_max,
			     sc_num);
	...
	get_detail_threshold("ScapRawData_High_Min_", true,
			     thr->scap_rawdata_hi_min, sc_num);
	get_detail_threshold("ScapRawData_High_Max_", true,
			     thr->scap_rawdata_hi_max, sc_num);
	get_detail_threshold("ScapRawData_Hov_Min_", true,
			     thr->scap_rawdata_hov_min, sc_num);
	get_detail_threshold("ScapRawData_Hov_Max_", true,
			     thr->scap_rawdata_hov_max, sc_num);
	...
	get_detail_threshold("Panel_Differ_Max_Tx", true, thr->panel_differ_max,
			     node_num);
	get_detail_threshold("NoistTestCoefficient_Tx", true, thr->noise_max,
			     node_num);
	return 0;
}
...
static void print_thr_mc_sc(void)
{
	...
	FTS_TEST_DBG("noise_max:%d,frame_num:%d,noise_mode:%d,polling:%d",
	...
	print_buffer(thr->rawdata_l_max, tdata->node.node_num,
		     tdata->node.rx_num);
	print_buffer(thr->tx_linearity_max, tdata->node.node_num,
		     tdata->node.rx_num);
	print_buffer(thr->rx_linearity_max, tdata->node.node_num,
		     tdata->node.rx_num);
	print_buffer(thr->mcap_cmb_min, tdata->node.node_num,
		     tdata->node.rx_num);
	print_buffer(thr->mcap_cmb_max, tdata->node.node_num,
		     tdata->node.rx_num);
	...
	print_buffer(thr->scap_cb_hov_max, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_off_cf_min, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_off_cf_max, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_on_cf_min, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_on_cf_max, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_hi_cf_min, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
	print_buffer(thr->scap_cb_hi_cf_max, tdata->sc_node.node_num,
		     tdata->sc_node.rx_num);
