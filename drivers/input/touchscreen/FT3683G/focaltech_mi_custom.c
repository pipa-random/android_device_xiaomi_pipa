...
int fts_init_lockdown_info(u8 *buf)
{
	...
	ret = fts_flash_read(lockdown_addr, lockdown_info,
			     sizeof(lockdown_info));
	if (ret < 0) {
...
static int fts_touch_edge_mode_set(int value)
{
	...
	if ((value != ORIENTATION_0_OR_180) &&
	    (value != NORMAL_ORIENTATION_90) &&
	...
	if (fts_data->edge_mode != value) {
		...
		if (ret >= 0) {
			...
			FTS_DEBUG("MODE_EDGE switch to %d successfully !",
				  fts_data->edge_mode);
		}
	}
	return ret;
