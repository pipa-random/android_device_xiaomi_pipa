...
struct upgrade_func *upgrade_func_list_3683g[] = {
	&upgrade_func_ft5672,
};
...
static int fts_pram_write_buf(struct fts_upgrade *upg, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		if ((upg->ts_data->bus_type == BUS_TYPE_SPI) &&
		    (upg->ts_data->bus_ver == BUS_VER_V2)) {
			...
			ret = fts_write(packet_buf,
					FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
			if (ret < 0) {
				FTS_ERROR("pramboot set write address(%d) fail",
					  i);
				return ret;
			}
...
int fts_fwupg_ecc_cal(u32 saddr, u32 len)
{
	...
	for (i = 0; i < packet_num; i++) {
		...
		bflag = fts_fwupg_check_flash_status(
			FTS_CMD_FLASH_STATUS_ECC_OK, FTS_RETRIES_ECC_CAL,
...
int fts_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
			...
			if (ret < 0) {
				FTS_ERROR(
					"pram/bootloader write 03 command fail");
				return ret;
			}

			fts_msleep(
				FTS_CMD_READ_DELAY); /* must wait, otherwise read wrong data */
			ret = fts_read(NULL, 0, buf + offset, packet_len);
			if (ret < 0) {
				FTS_ERROR(
					"pram/bootloader read 03 command fail");
				return ret;
			}
			...
			if (upg->ts_data->bus_ver == BUS_VER_V2) {
				...
				wbuf[0] = FTS_CMD_READ;
				ret = fts_read(wbuf, 1, buf + offset,
					       packet_len);
				if (ret < 0) {
					FTS_ERROR(
						"pram/bootloader read 03(SPI_V2) command fail");
					return ret;
				}
				...
				if (ret < 0) {
					FTS_ERROR(
						"pram/bootloader read 03(SPI) command fail");
					return ret;
				}
			}
		} else
			FTS_ERROR("unknown bus type:%d",
				  upg->ts_data->bus_type);
	}
...
static int fts_fwupg_get_moduleid(struct fts_upgrade *upg, int *module_id)
{
	...
	if (fwvalid) {
		...
		if (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_1) {
			...
			ret = fts_flash_read(fwcfg_addr, cfgbuf,
					     FTS_HEADER_LEN);
		}
