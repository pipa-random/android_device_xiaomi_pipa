...
#define XM_HAP_REGISTER_EXCEPTION(type, keyword)                               \
	xm_hap_driver_exception_handler(XM_HAP_REGISTER_EXCEPTION_CODE, type,  \
					keyword)
#define XM_HAP_HBOOST_EXCEPTION(raceObject, keyword)                           \
	xm_hap_driver_exception_handler(XM_HAP_HBOOST_EXCEPTION_CODE,          \
					raceObject, keyword)
#define XM_HAP_F0_PROTECT_EXCEPTION(calVal, keyword)                           \
	xm_hap_driver_exception_handler(XM_HAP_F0_PROTECT_EXCEPTION_CODE,      \
					calVal, keyword)
#define XM_HAP_F0_CAL_EXCEPTION(calVal, defaultVal, keyword)                   \
	xm_hap_driver_exception_handler(XM_HAP_F0_CAL_EXCEPTION_CODE, calVal,  \
					defaultVal, keyword)
...
static void xm_hap_driver_exception_handler(int exception_code, ...)
{
	...
	case XM_HAP_REGISTER_EXCEPTION_CODE:
		XM_HAP_REGISTER_EXCEPTION_EVENT(event, va_arg(args, char *),
						va_arg(args, char *));
		break;
	case XM_HAP_HBOOST_EXCEPTION_CODE:
		XM_HAP_HBOOST_EXCEPTION_EVENT(event, va_arg(args, char *),
					      va_arg(args, char *));
		break;
	case XM_HAP_F0_PROTECT_EXCEPTION_CODE:
		XM_HAP_F0_PROTECT_EXCEPTION_EVENT(event, va_arg(args, int),
						  va_arg(args, char *));
		break;
	case XM_HAP_F0_CAL_EXCEPTION_CODE:
		XM_HAP_F0_CAL_EXCEPTION_EVENT(event, va_arg(args, int),
					      va_arg(args, int),
					      va_arg(args, char *));
		break;
	default:
