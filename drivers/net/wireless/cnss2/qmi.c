...
#define ELF_BDF_FILE_NAME_M80 "bd_m80.elf"
#define ELF_BDF_FILE_NAME_M80_GF "bd_m80gf.elf"
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK) {
				if (hw_platform_ver == HARDWARE_PROJECT_L1 ||
				...
				} else if (hw_platform_ver ==
					   HARDWARE_PROJECT_L2S) {
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME_L2S_GF);
				} else if (hw_platform_ver ==
					   HARDWARE_PROJECT_L3S) {
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME_L3S_GF);
				} else if (hw_platform_ver ==
					   HARDWARE_PROJECT_L2) {
					if ((uint32_t)CountryGlobal ==
					    hw_country_ver)
						snprintf(
							filename_tmp,
							filename_len,
					...
					if ((uint32_t)CountryGlobal ==
					   ...
					   HARDWARE_PROJECT_L18) {
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME_L18_GF);
				} else if (hw_platform_ver ==
					   HARDWARE_PROJECT_M11A) {
					if ((uint32_t)CountryGlobal ==
					    hw_country_ver)
						snprintf(
							filename_tmp,
					...
					if ((uint32_t)CountryGlobal ==
					   ...
					   HARDWARE_PROJECT_M80) {
					snprintf(filename_tmp, filename_len,
						 ELF_BDF_FILE_NAME_M80_GF);
				} else
					snprintf(filename_tmp, filename_len,
			...
			} else if (hw_platform_ver == HARDWARE_PROJECT_M80) {
				snprintf(filename_tmp, filename_len,
					 ELF_BDF_FILE_NAME_M80);
			} else
				snprintf(filename_tmp, filename_len,
					 ELF_BDF_FILE_NAME);
