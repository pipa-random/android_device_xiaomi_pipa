...
enum qdf_suspend_type { QDF_SYSTEM_SUSPEND, QDF_RUNTIME_SUSPEND };

enum wow_cloud_ctrl_type { WOW_CLOUD_CTRL_DISABLED, WOW_CLOUD_CTRL_ENABLED };
...
static struct nla_policy
	cnss_statistic_genl_policy[CNSS_STATISTIC_GENL_ATTR_MAX + 1] = {
...
static int cnss_statistic_genl_debug(struct sk_buff *skb,
...
static int cnss_statistic_genl_enable(struct sk_buff *skb,
				      struct genl_info *info)
{
...
static int cnss_statistic_genl_disable(struct sk_buff *skb,
				       struct genl_info *info)
{
...
int cnss_statistic_wow_wakeup(u16 proto_subtype, u16 src_port, u16 target_port)
{
	...
	ret = nla_put_u16(msg, CNSS_STATISTIC_GENL_ATTR_PROTO_SUBTYPE,
			  proto_subtype);
	if (ret < 0)
	...
	genlmsg_end(msg, hdr);
	ret = genlmsg_multicast(&cnss_statistic_genl_family, msg, 0, 0,
				GFP_ATOMIC);
	if (ret < 0)
