...
#define PHY_USB3_TUNING_REG_LEN 9 //add dtsi length
...
static int msm_ssphy_qmp_init(struct usb_phy *uphy)
{
	...
	if (phy->usb3_eye) {
		//reset reg addr
		reg = reg - PHY_USB3_TUNING_REG_LEN;

		if (phy->TXA_DRV_LVL) {
			dev_err(uphy->dev, "TXA_DRV_LVL:0x%02x.\n",
				phy->TXA_DRV_LVL);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF, phy->TXA_DRV_LVL);
			else
				dev_err(uphy->dev, "write TXA_DRV_LVL fail.\n");
		}
		reg++;
		if (phy->TXB_DRV_LVL) {
			dev_err(uphy->dev, "TXB_DRV_LVL:0x%02x.\n",
				phy->TXB_DRV_LVL);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF, phy->TXB_DRV_LVL);
			else
				dev_err(uphy->dev, "write TXB_DRV_LVL fail.\n");
		}
		reg++;
		if (phy->TXA_PRE_EMPH) {
			dev_err(uphy->dev, "TXA_PRE_EMPH:0x%02x.\n",
				phy->TXA_PRE_EMPH);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
				...
				dev_err(uphy->dev,
					"write TXA_PRE_EMPH fail.\n");
		}
		reg++;
		if (phy->TXB_PRE_EMPH) {
			dev_err(uphy->dev, "TXB_PRE_EMPH:0x%02x.\n",
				phy->TXB_PRE_EMPH);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
				...
				dev_err(uphy->dev,
					"write TXB_PRE_EMPH fail.\n");
		}
		reg++;
		if (phy->TXA_POST1_LVL) {
			dev_err(uphy->dev, "TXA_POST1_LVL:0x%02x.\n",
				phy->TXA_POST1_LVL);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF,
				...
				dev_err(uphy->dev,
					"write TXA_POST1_LVL fail.\n");
		}
		reg++;
		if (phy->TXB_POST1_LVL) {
			dev_err(uphy->dev, "TXB_POST1_LVL:0x%02x.\n",
				phy->TXB_POST1_LVL);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF,
				...
				dev_err(uphy->dev,
					"write TXB_POST1_LVL fail.\n");
		}
		reg++;
		if (phy->TXMGN_V0) {
			dev_err(uphy->dev, "TXMGN_V0:0x%02x.\n", phy->TXMGN_V0);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF, phy->TXMGN_V0);
			else
		...
		if (phy->TXDEEMPH_M3P5DB) {
			dev_err(uphy->dev, "TXDEEMPH_M3P5DB:0x%02x.\n",
				phy->TXDEEMPH_M3P5DB);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF,
				...
				dev_err(uphy->dev,
					"write TXDEEMPH_M3P5DB fail.\n");
		}
		reg++;
		if (phy->G3S2_TXMGN_MAIN) {
			dev_err(uphy->dev, "G3S2_TXMGN_MAIN:0x%02x.\n",
				phy->G3S2_TXMGN_MAIN);
			if (reg && reg->offset != -1)
				msm_usb_write_readback(phy->base, reg->offset,
						       0xFF,
		...
		for (i = 0; i < PHY_USB3_TUNING_REG_LEN; i++) {
			dev_err(uphy->dev, "CRC USB3 PHY CFG:%d:0x%02x.\n", i,
				(0xFF &
				 (readl_relaxed(phy->base + reg->offset))));
			reg++;
		}
	}
...
static void msm_ssphy_create_debugfs(struct msm_ssphy_qmp *phy)
{
	...
	debugfs_create_x8("txdeemph_m3p5db", 0644, phy->root,
			  &phy->TXDEEMPH_M3P5DB);
	debugfs_create_x8("g3s2_txmgn_main", 0644, phy->root,
			  &phy->G3S2_TXMGN_MAIN);
}
